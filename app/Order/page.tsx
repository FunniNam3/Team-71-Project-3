  ...
  useEffect(() => {
    ...
    Promise.all([fetchDrinks, fetchFoods])
      .then(([drinksRes, foodsRes]) => {
        if (drinksRes.data) setDrinkItems(drinksRes.data.map((item: any) => ({
          ...item, imageUrl: "/menu/" + String(item.name).trim() + ".png", category: item.category || "milk tea"
        })));
        if (foodsRes.data) setFoodItems(foodsRes.data.map((item: any) => ({
          ...item, imageUrl: "/menu/" + String(item.name).trim() + ".png", category: "food"
        })));
      })
